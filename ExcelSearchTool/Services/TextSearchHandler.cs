...
public sealed class TextSearchHandler(ErrorLogger errorLogger) : ISearchHandler
{
    ...
    public async Task SearchAsync(FileInfo file, SearchContext context)
    {
        ...
        catch (DecoderFallbackException)
        {
            ...
            {
                await _errorLogger.LogAsync($"Text file skipped due to encoding/read error: {file.FullName}", ex).ConfigureAwait(false);
            }
        }
        ...
        {
            await _errorLogger.LogAsync($"Text file skipped due to read error: {file.FullName}", ex).ConfigureAwait(false);
        }
    }
